        ...
        with psycopg.connect(db_url) as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    ...
                    (str(uuid.uuid4()), token_hash, expires_at, datetime.now(), user_id)
                )
...
def main():
    try:
        with psycopg.connect(db_url) as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT email FROM users_user WHERE email LIKE 'test_reset_%' ORDER BY created_at DESC LIMIT 1")
