...
class ConnectLaundryQA:
    ...
    def log(self, action, success=True, message="", response=None):
        ...
        if not success and response is not None:
            try:
                ...
                print(
                    f"      Response ({response.status_code}): {response.text[:1000]}")
    ...
    def test_auth(self):
        ...
        phone_c = "055" + "".join([str(random.randint(0, 9))
                                  for _ in range(7)])
        cust_data = owner_data.copy()
        cust_data.update(
            {"email": email_c, "phone": phone_c, "role": "CUSTOMER"})
    ...
    def test_laundry_management(self):
        ...
        res = self.run_request(
            "POST",
            "/laundries/dashboard/my-laundry/",
            laundry_data,
            self.tokens['owner'])
        ...
        res = self.run_request("PATCH",
                               f"/laundries/admin/laundries/{self.l_id}/approve/",
                               {},
                               self.tokens['admin'])
        ...
        if self.item_id and self.category_id:
            ...
            if res is not None and res.status_code == 201:
                ...
                res = self.run_request("PATCH",
                                       f"/laundries/dashboard/my-laundry/{self.l_id}/",
                                       update_data,
                                       self.tokens['owner'])
                self.log(
                    "Enabled PER_ITEM",
                    res is not None and res.status_code == 200)
            else:
                self.log("Service Addition", False, f"Failed", res)

    def test_order_lifecycle(self):
        if not self.l_id or 'customer' not in self.tokens:
            return
        ...
        res = self.run_request(
            ...
            "/booking/create/",
            order_data,
            self.tokens['customer'])
        if res is not None and res.status_code in [200, 201]:
            resp_json = res.json()
            self.order_id = resp_json.get(
                'data', {}).get('id') or resp_json.get('id')
        ...
        res = self.run_request("PATCH",
                               f"/orders/{self.order_id}/update-weight/",
                               weight_data,
                               self.tokens['owner'])
        if res is not None and res.status_code == 200:
            data = res.json().get('data', res.json())
            self.log(
                "Update Weight", True, f"Final Price: {
                    data.get('final_price')}, Status: {
                    data.get('status')}")
        else:
            self.log("Update Weight", False, "Failed", res)

        res = self.run_request("GET",
