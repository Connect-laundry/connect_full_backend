...
if LAUNDRY_ID:
    test(
        ...
        200,
        data={"description": "Updated: Best in Accra!"},
        headers=AUTH,
    )
    test(
        "Get Hours",
...
resp = test(
    ...
    f"{BASE}/laundries/dashboard/machines/",
    201,
    data={"name": "Samsung Washer #1", "machine_type": "WASHER", "notes": "12kg"},
    headers=AUTH,
)
MACHINE_ID = (
    resp.json().get("data", {}).get("id") if resp and resp.status_code == 201 else None
)
...
test(
    ...
    f"{BASE}/laundries/dashboard/machines/",
    201,
    data={"name": "LG Dryer #1", "machine_type": "DRYER"},
    headers=AUTH,
)
...
if MACHINE_ID:
    ...
    test(
        ...
        200,
        data={"notes": "Serviced 2026-03-25"},
        headers=AUTH,
    )
    test(
        "Status -> MAINTENANCE",
...
test(
    "List Staff (empty)", "get", f"{BASE}/laundries/dashboard/staff/", 200, headers=AUTH
)
...
test(
    ...
    f"{BASE}/laundries/dashboard/staff/invite/",
    201,
    data={"name": "Ama Owusu", "email": "ama@laundry.com", "role": "MANAGER"},
    headers=AUTH,
)
...
test(
    ...
    f"{BASE}/laundries/dashboard/staff/invite/",
    400,
    data={"name": "Kofi", "email": "kofi@laundry.com", "role": "WASHER"},
    headers=AUTH,
)
...
test(
    "Customer List", "get", f"{BASE}/laundries/dashboard/customers/", 200, headers=AUTH
)
...
if BANK_ID:
    test(
        ...
        400,
        data={"bank_account_id": BANK_ID, "amount": "100.00"},
        headers=AUTH,
    )
...
test(
    "Dashboard Orders", "get", f"{BASE}/laundries/dashboard/orders/", 200, headers=AUTH
)
