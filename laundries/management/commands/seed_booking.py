    ...
    def handle(self, *args, **kwargs):
        ...
        if not laundries.exists():
            self.stdout.write(self.style.ERROR(
                'No laundries found. Run seed_laundries first.'))
            return
        ...
        if not services.exists():
            self.stdout.write(self.style.ERROR(
                'No service types found. Populate Categories first.'))
            return
        ...
        for laundry in laundries:
            ...
            for item in offered_items:
                ...
                offered_services = random.sample(
                    list(services), min(
                        len(services), random.randint(
                            1, 3)))

                for service in offered_services:
                    base_price = Decimal(
                        str(random.randint(15, 60)))  # nosec B311
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'Successfully created {slots_count} booking slots!'))
