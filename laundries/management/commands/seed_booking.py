...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not laundries.exists():
            self.stdout.write(
                self.style.ERROR("No laundries found. Run seed_laundries first.")
            )
            return
        ...
        if not items.exists():
            self.stdout.write(
                self.style.ERROR(
                    "No global items found. Populate LaunderableItems first."
                )
            )
            return
        ...
        if not services.exists():
            self.stdout.write(
                self.style.ERROR("No service types found. Populate Categories first.")
            )
            return
        ...
        for laundry in laundries:
            ...
            offered_items = random.sample(
                list(items), min(len(items), random.randint(5, 10))
            )

            for item in offered_items:
                # Randomly pick 1-3 services for this item
                offered_services = random.sample(
                    list(services), min(len(services), random.randint(1, 3))
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully created {seeded_count} price configurations!"
            )
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Successfully created {slots_count} booking slots!")
        )
