...
class LaundryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if (
            ...
            if user.is_authenticated:
                if user.is_staff:
                    ...
                    laundry_owner_exists = Laundry.objects.filter(
                        id=laundry_id, owner=user
                    ).exists()
                    show_all_services = laundry_owner_exists

            service_filter = Q(is_active=True)
            if not show_all_services:
                # Note: LaundryService model uses 'is_available', not
                # 'is_approved'
                    ...
                    [
                        Prefetch(
                            ...
                            queryset=LaundryService.objects.filter(
                                is_available=True
                            ).select_related("service_type", "item"),
                        ),
                        "reviews__user",
                    ]
                )
        ...
        if USE_POSTGIS:
            ...
            lat = self.request.query_params.get("lat") or self.request.query_params.get(
                "latitude"
            )
            lng = self.request.query_params.get("lng") or self.request.query_params.get(
                "longitude"
            )
            radius_km = 10
            try:
            ...
            if nearby and lat and lng:
                try:
                    ...
                    logger.info(
                        f"Spatial search triggered for ({lat}, {lng}) within {radius_km}km"
                    )
        ...
        if recommended:
            ...
            queryset = queryset.annotate(
                ...
                score=ExpressionWrapper(
                    F("safe_rating") * F("reviewsCount"), output_field=FloatField()
                ),
        ...
        # 6. Featured Filter
        if (
            self.request.query_params.get("is_featured") == "true"
            or self.request.query_params.get("featured") == "true"
        ):
    ...
    def featured(self, request):
        ...
        try:
            ...
            logger.error(
                f"Critical error in featured laundries endpoint: {e}", exc_info=True
            )
            return Response(
                {
                    "success": False,
            ...
            return Response(
                {
                    "success": False,
                    "status": "error",
                    "message": "You do not have permission to deactivate this laundry.",
                },
            ...
            return Response(
                {
                    "success": False,
                    "status": "error",
                    "message": "Laundry is already inactive",
                },
                ...
                return Response(
                    {
                        "success": False,
                        "status": "error",
                        "message": "You do not have permission to manage services for this laundry.",
                    },
                    status=status.HTTP_403_FORBIDDEN,
                )
