...
class LaundryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Laundry.objects.filter(
            ...
            is_active=True).select_related('owner').annotate(
            ...
            if user.is_authenticated:
                if user.is_staff:
                    ...
                    laundry_owner_exists = Laundry.objects.filter(
                        id=laundry_id, owner=user).exists()
        ...
        if USE_POSTGIS:
            ...
            lat = self.request.query_params.get(
                'lat') or self.request.query_params.get('latitude')
            lng = self.request.query_params.get(
                'lng') or self.request.query_params.get('longitude')
            radius_km = 10
            try:
        ...
        if self.request.query_params.get(
                'is_featured') == 'true' or self.request.query_params.get('featured') == 'true':
    ...
    def featured(self, request):
        ...
        try:
            ...
            logger.error(
                f"Critical error in featured laundries endpoint: {e}",
                exc_info=True)
            return Response({
                "success": False,
