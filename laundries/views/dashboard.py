...
class DashboardEarningsView(views.APIView, DashboardBaseView):
    ...
    def get(self, request):
        ...
        earnings = completed_orders.aggregate(
            ...
            this_week=Sum(
                "final_price", filter=Q(created_at__gte=week_start), default=0
            ),
            this_month=Sum(
                "final_price", filter=Q(created_at__gte=month_start), default=0
            ),
        ...
        time_series = list(
            completed_orders.filter(created_at__gte=now - timedelta(days=12))
