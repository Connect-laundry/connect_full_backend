...
class DashboardStatsView(views.APIView, DashboardBaseView):
    ...
    def get(self, request):
        ...
        # Add recent activity
        stats['recent_orders'] = Order.objects.filter(
            laundry=laundry).select_related('user').order_by('-created_at')[:5]
        stats['recent_reviews'] = Review.objects.filter(
            laundry=laundry).select_related('user').order_by('-created_at')[:5]
...
class DashboardEarningsView(views.APIView, DashboardBaseView):
    ...
    def get(self, request):
        ...
        earnings = completed_orders.aggregate(
            today=Sum(
                'final_price', filter=Q(
                    created_at__gte=today_start), default=0), this_week=Sum(
                'final_price', filter=Q(
                    created_at__gte=week_start), default=0), this_month=Sum(
                        'final_price', filter=Q(
                            created_at__gte=month_start), default=0), total_revenue=Sum(
                                'final_price', default=0))
        ...
        positive_reviews = Review.objects.filter(
            laundry=laundry, rating__gte=4).count()
        sentiment_score = round(
            (positive_reviews / total_reviews) * 100,
            1) if total_reviews > 0 else None
