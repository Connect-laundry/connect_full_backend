...
class OwnerLaundryAPITests(APITestCase):
    ...
    def create_test_laundry(
        self, name, status=Laundry.ApprovalStatus.PENDING, is_active=False
    ):
    ...
    def test_toggle_store_status(self):
        ...
        laundry = self.create_test_laundry(
            name="Toggle Test", status=Laundry.ApprovalStatus.APPROVED, is_active=False
        )
    ...
    def test_toggle_pending_store_fails(self):
        ...
        laundry = self.create_test_laundry(
            name="Pending Test", status=Laundry.ApprovalStatus.PENDING, is_active=False
        )
        url = reverse("owner-laundry-toggle", kwargs={"pk": laundry.id})
        ...
        self.assertIn(
            "Only approved laundries can be toggled", response.data["message"]
        )
    ...
    def test_get_owner_reviews(self):
        ...
        Review.objects.create(
            user=self.customer, laundry=laundry, rating=5, comment="Great service!"
        )
        Review.objects.create(
            user=self.customer, laundry=laundry, rating=4, comment="Good."
        )
        ...
        self.assertEqual(
            response.data["results"][0]["rating"], 4
        )  # Order by -created_at usually
