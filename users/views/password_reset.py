        ...
        if serializer.is_valid():
            ...
            if user:
                ...
                try:
                    ...
                    # Log the failure but don't return 500 (silent failure for
                    # security & stability)
                    logger.error(f"Failed to trigger password reset email for {email}: {
                            str(e)}")
...
class ResetPasswordView(views.APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                token_record = PasswordResetToken.objects.get(token_hash=token_hash)
