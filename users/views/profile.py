...
class LogoutView(APIView):
    ...
    def post(self, request):
        try:
            ...
            refresh_token = request.data.get("refreshToken") or request.data.get(
                "refresh"
            )
...
class SupportedCitiesView(APIView):
    ...
    def get(self, request):
        ...
        cities = (
            Laundry.objects.filter(is_active=True, status="APPROVED")
            .values_list("city", flat=True)
            .distinct()
        )
        return Response({"status": "success", "cities": list(cities)})
