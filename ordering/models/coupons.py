...
class Coupon(models.Model):
    ...
    max_usage = models.PositiveIntegerField(
        null=True, blank=True, help_text=_("Total times this coupon can be used.")
    )
    current_usage = models.PositiveIntegerField(default=0)

    user_limit = models.PositiveIntegerField(
        default=1, help_text=_("Times a single user can use this coupon.")
    )
    ...
    applicable_laundries = models.ManyToManyField(
        "laundries.Laundry", blank=True, related_name="coupons"
    )
    ...
    def is_valid(self, user=None, laundry_id=None, order_value=0):
        ...
        if Decimal(str(order_value)) < self.min_order_value:
            return False, _(f"Minimum order value of {
                    self.min_order_value} required.")

        if self.applicable_laundries.exists():
            if (
                not laundry_id
                or not self.applicable_laundries.filter(id=laundry_id).exists()
            ):
        ...
        if user:
            ...
            if self.first_time_users_only:
                ...
                if Order.objects.filter(user=user, status="COMPLETED").exists():
                    return False, _(
                        "This coupon is only available for first-time orders."
                    )
...
class CouponUsage(models.Model):
    ...
    order = models.OneToOneField(
        "ordering.Order", on_delete=models.CASCADE, related_name="coupon_usage"
    )
