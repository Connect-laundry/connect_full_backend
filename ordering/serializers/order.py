...
class LaunderableItemSerializer(serializers.ModelSerializer):
    item_category_name = serializers.CharField(
        source='item_category.name', read_only=True)
...
class OrderDetailSerializer(serializers.ModelSerializer):
    ...
    def _get_latest_tracking_coord(self, obj, coord_type):
        ...
        if obj.status == Order.Status.OUT_FOR_DELIVERY:
            ...
            log = TrackingLog.objects.filter(
                order=obj).order_by('-timestamp').first()
            if log:
...
class OrderCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if laundry and (laundry.status != 'APPROVED' or not laundry.is_active):
            raise serializers.ValidationError(
                "This laundry is not approved or is currently inactive.")
        ...
        try:
            ...
            error_data = e.message_dict if hasattr(
                e, 'message_dict') else str(e)
            raise serializers.ValidationError({
        ...
        if pricing_method == 'PER_KG':
            if not laundry.price_per_kg:
                raise serializers.ValidationError(
                    f"{laundry.name} does not support weight-based pricing.")
            if not data.get('estimated_weight'):
                raise serializers.ValidationError(
                    {"estimated_weight": "Estimated weight is required for Per Kg orders."})
        else:
            if not data.get('items'):
                raise serializers.ValidationError(
                    {"items": "Items are required for Per Item orders."})
        ...
        if coupon_code:
            ...
            try:
                ...
                is_valid, error = coupon.is_valid(
                    user=user, laundry_id=laundry.id if laundry else None)
                if not is_valid:
    ...
    def create(self, validated_data):
        ...
        if pricing_method == 'PER_ITEM':
            ...
            for item_data in items_data:
                ...
                if not service_type_instance:
                    raise serializers.ValidationError(
                        f"Service type is required for {
                            item_instance.name}")
                ...
                try:
                    ...
                    if not l_service.is_available:
                        raise serializers.ValidationError(
                            f"{item_instance.name} is not available for {service_type_instance.name}.")
                    item_price = l_service.price
                except LaundryService.DoesNotExist:
                    raise serializers.ValidationError(
                        f"Laundry does not offer {
                            service_type_instance.name} for {
                            item_instance.name}")
        ...
        price_breakdown = FinanceService.calculate_price_breakdown(
        ...
        if coupon_obj:
            ...
            CouponUsage.objects.create(
                user=user, coupon=coupon_obj, order=order)
