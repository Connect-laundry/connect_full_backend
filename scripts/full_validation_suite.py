...
def assert_status(response, expected_status, test_name):
    passed = (isinstance(expected_status, list) and response.status_code in expected_status) or (not isinstance(expected_status, list) and response.status_code == expected_status)
...
def run_tests():
    ...
    try:
        ...
        logging.info("\n--- STEP 3: Laundry Lifecycle ---")
        r3 = o_client.post(f"{BASE_URL}/laundries/dashboard/my-laundry/", json={
        ...
        # Admin Approval
        r4 = a_client.patch(f"{BASE_URL}/laundries/admin/laundries/{store['laundry_id']}/approve/")
        assert_status(r4, 200, "3.2 Admin Approval")
        
        # STEP 4: Configuration
        logging.info("\n--- STEP 4: Configuration ---")
        # Hours
        r5 = o_client.put(f"{BASE_URL}/laundries/dashboard/my-laundry/{store['laundry_id']}/hours/", json=[
            {"day": i, "opening_time": "08:00:00", "closing_time": "18:00:00"} for i in range(1, 8)
        ])
        assert_status(r5, 200, "4.1 Set Operating Hours")
        
        # Add Service Item (to allow PER_ITEM activation)
        r_item = o_client.post(f"{BASE_URL}/laundries/dashboard/my-laundry/{store['laundry_id']}/services/", json={
            "item_id": ITEM_ID,
        ...
        # STEP 5: Activation
        logging.info("\n--- STEP 5: Activation ---")
        r6 = o_client.patch(f"{BASE_URL}/laundries/dashboard/my-laundry/{store['laundry_id']}/activate/")
        assert_status(r6, 200, "5.1 Successful Activation")

        # STEP 6: Order - PER_KG
        logging.info("\n--- STEP 6: Ordering - PER_KG ---")
        r7 = c_client.post(f"{BASE_URL}/booking/create/", json={
        ...
        r8 = c_client.post(f"{BASE_URL}/booking/create/", json={
            "laundry": store["laundry_id"],
            "pickup_date": "2026-10-11T10:00:00Z",
            "pickup_address": "Test Street 11",
            "pricing_method": "PER_ITEM",
            "items": [
                {"item": ITEM_ID, "service_type": ST_ID, "quantity": 2}
            ]
        })
        ...
        if order_id_kg:
            r9 = c_client.patch(f"{BASE_URL}/orders/{order_id_kg}/update-weight/", json={"actual_weight": 4.5})
    ...
    logging.info("="*40)
    logging.info(json.dumps({"total": results["total_tests"], "passed": results["passed"], "failed": results["failed"]}, indent=2))
